...
if($_SESSION['emp_type']=="admin" ||  $_SESSION['emp_type']=="hr"){
$sql="select * from `print_application` WHERE `printedby_id`='$emp_session_id'";



}
elseif($_SESSION['emp_type']=="superadmin"){
...
//if employee type is admin then show all appliaction else show only login user appliaction 
    ...
    elseif($_SESSION['emp_type']=="superadmin"){
        ...
        $sql1="SELECT * FROM `print_application` ORDER BY  `id` DESC LIMIT $offset,$limit";
        $query1=mysqli_query($conn,$sql1);
        $numres=mysqli_num_rows($query1);
        if($numres<=0){
         $_SESSION['msg2']="No Data Found";
      ...
      <!-- loop to show all record one by one  -->
                   ...
                   $r=$offset+1;
                    ...
                    <tr>
                     ...
                     <td><?php echo $row['printedby_name'];?>/<?php echo $row['printedby_id']; ?></td>
                      ...
                      <td>
                        <div class="d-flex">
                        <a class="btn btn-warning mx-2" href="../operation/delete_print_application.php?id=<?php echo $row['id']; ?>">Delete</a>

                         

                            <?php
                            if($_SESSION['emp_type']=="superadmin"){ ?>



                                     <!-- <a href="select_card.php?id=<?php //echo $row['emp_id']; ?>" class="btn btn-success mx-1"><b>Print</b></a> -->



                              <?php if ($row['application_status']=='yes') { ?>
                                        <form method="post">
                              ...
                              <?php }elseif($row['application_status']=='no'){ ?>
                                     <form method="post">
                           ...
                           <?php }elseif($_SESSION['emp_type']=="admin" || $_SESSION['emp_type']=="hr"){
          ...
          if($per_page>0){ ?>
                <div class="row col-md-12 my-3">
                  <nav aria-label="Page navigation example">
                      <ul class="pagination">
                        <li class="page-item">
                          <a class="page-link" href="print_application_list.php?page=<?php echo 1;?>" aria-label="Previous">
                            <span aria-hidden="true">&laquo;</span>
                          </a>
                        </li>
                        <?php for ($p=1; $p<=$per_page ; $p++) { ?>
                        
                        <li class="page-item"><a class="page-link" href="print_application_list.php?page=<?php echo $p; ?>"><?php echo $p; ?></a></li>
