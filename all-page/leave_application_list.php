...
<?php
  ...
  $active_link_application_list="active";
...
if($_SESSION['emp_type']=="admin" || $_SESSION['emp_type']=="superadmin" || $_SESSION['emp_type']=="hr"){
  ...
  $sql="select * from `leave_application` WHERE `emp_id`='$emp_session_id'";
...
//if employee type is admin then show all appliaction else show only login user appliaction 
    ...
    if($_SESSION['emp_type']=="admin" || $_SESSION['emp_type']=="superadmin" || $_SESSION['emp_type']=="hr"){
     ...
     $sql1="SELECT * FROM `leave_application` ORDER BY  `emp_name` DESC LIMIT $offset,$limit";
     ...
     if($numres<=0){
      $_SESSION['msg2']="No Data Found";
       ...
       $sql1="SELECT * FROM `leave_application`  WHERE `emp_id`='$emp_session_id' ORDER BY  `emp_name` DESC LIMIT $offset,$limit ";
      $query1=mysqli_query($conn,$sql1);
      $numres=mysqli_num_rows($query1);
       if($numres<=0){
        $_SESSION['msg2']="No Data Found";
...
if(isset($_POST['approved_submit'])){
      ...
      $leave_status=$_POST['approved_input'];

      $sql2="UPDATE `leave_application`  SET `leave_status`='$leave_status'  WHERE `leave_id`='$leave_id'";
...
<!-- thsi is for show error massage  -->
          ...
          <div class="row mt-2 bg-secondary py-2 col-md-12">
            <div class="col-md-10">
              <h3 class="text-center text-light">All Leave Application</h3>
            </div>
            <div class="col-md-2">
              <a href="leave_application_list.php" class="btn btn-info"><i class="fa fa-refresh mx-1"></i>Refresh List</a>
            </div>
      ...
      <!-- loop to show all record one by one  -->
                    ...
                    <tr>
                      ...
                      <td>
                        <div class="d-flex">
                           <a class="btn btn-danger mx-2" href="../operation/delete_leave_application.php?id=<?php echo $row['leave_id'];?>">Delete</a>
                            ...
                            if($_SESSION['emp_type']=="admin" || $_SESSION['emp_type']=="superadmin" || $_SESSION['emp_type']=="hr"){
                                      ...
                                      if ($row['leave_status']=='approved') { ?>
                                        <form method="post">
                                           <input type="hidden" name="approved_input" value="not approved">
                                    ...
                                    <?php
                                     ...
                                     <form method="post">
                                           <input type="hidden" name="approved_input" value="approved">
                                           <input type="hidden" name="leave_id" value="<?php echo $row['leave_id']; ?>">
                                   ...
                                   if ($row['leave_status']=='approved') { ?>
                                            <span class="badge bg-success text-center">Approved</span>
                                    <?php
                                   }elseif($row['leave_status']=='not approved'){ ?>
                                        <span class="badge bg-danger text-center">Not Approved</span>
                                    <?php }
          ...
          if($per_page>0){ ?>
                <div class="row col-md-12 my-3">
                  <nav aria-label="Page navigation example">
                      <ul class="pagination">
                        <li class="page-item">
                          <a class="page-link" href="leave_application_list.php?page=<?php echo 1;?>" aria-label="Previous">
                            <span aria-hidden="true">&laquo;</span>
                          </a>
                        </li>
                        <?php for ($p=1; $p<=$per_page ; $p++) { ?>
                        
                        <li class="page-item"><a class="page-link" href="leave_application_list.php?page=<?php echo $p; ?>"><?php echo $p; ?></a></li>
                      ...
                      ?>
                        <li class="page-item">
                          <a class="page-link" href="leave_application_list.php?page=<?php echo $per_page; ?>" aria-label="Next">
